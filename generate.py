...
def _validate_args(args):
    ...
    if "animate" in args.task:
        ...
        assert args.replacement_conditioning_mode in {"legacy", "rich", "rich_v1", "semantic_v1", "decoupled_v1", "decoupled_v2", "core_rich_v1"}, (
            "--replacement_conditioning_mode must be one of: legacy, rich, rich_v1, semantic_v1, decoupled_v1, decoupled_v2, core_rich_v1."
        )
        assert args.boundary_refine_mode in {"none", "deterministic", "v2", "roi_v1", "semantic_v1", "semantic_experts_v1", "local_edge_v1", "roi_gen_v1", "roi_gen_v2"}, (
            "--boundary_refine_mode must be one of: none, deterministic, v2, roi_v1, semantic_v1, semantic_experts_v1, local_edge_v1, roi_gen_v1, roi_gen_v2."
        )
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="legacy",
        choices=["legacy", "rich", "rich_v1", "semantic_v1", "decoupled_v1", "decoupled_v2", "core_rich_v1"],
        help="How Wan-Animate replacement consumes richer preprocess signals. 'rich_v1' uses alpha_v2/trimap/fine-boundary/hair-edge plus uncertainty/occlusion/background confidence/face preservation maps. 'semantic_v1' additionally uses semantic boundary subclasses (face/hair/hand/cloth/occluded). 'decoupled_v1' uses explicit foreground/background contract artifacts when available. 'decoupled_v2' promotes those decoupled artifacts deeper into core conditioning and background-release logic. 'core_rich_v1' promotes richer boundary signals into a stronger background/foreground core conditioning RGB path. 'rich' is kept as a legacy alias."
    )
    parser.add_argument(
