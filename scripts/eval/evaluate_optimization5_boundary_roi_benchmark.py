...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Optimization5 Step05 boundary ROI reconstruction benchmark.")
    ...
    seam_after = float(rows["roi_gen_v1"].get("seam_score_mean", 0.0) or 0.0)
    bg_before = float(rows["none"].get("background_fluctuation_mean", 0.0) or 0.0)
    bg_after = float(rows["roi_gen_v1"].get("background_fluctuation_mean", 0.0) or 0.0)
    ...
    result = {
        ...
        "gates": {
            ...
            "roi_gradient_gain_ge_10pct": gradient_gain_pct >= 10.0,
