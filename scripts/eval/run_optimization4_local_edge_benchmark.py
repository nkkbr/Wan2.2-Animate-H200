...
LOCAL_EDGE_METRICS_SCRIPT = REPO_ROOT / "scripts" / "eval" / "compute_local_edge_restoration_metrics.py"
...
def main():
    parser = argparse.ArgumentParser(description="Run optimization4 Step 07 local edge restoration AB.")
    ...
    parser.add_argument("--boundary_refine_strength", type=float, default=0.42)
    ...
    suite_name = args.suite_name or f"optimization4_step07_local_edge_{timestamp}"
    ...
    roi_output = None
    for boundary_refine_mode in ("none", "roi_v1", "local_edge_v1"):
        ...
        if generate.returncode == 0 and output_video is not None:
            ...
            if boundary_refine_mode == "none":
                baseline_output = output_video
            elif boundary_refine_mode == "roi_v1":
                roi_output = output_video
            elif boundary_refine_mode == "local_edge_v1" and roi_output is not None:
                boundary_metrics_path = suite_dir / "local_edge_v1_boundary_metrics.json"
                boundary = _run([
                    PYTHON, str(BOUNDARY_METRICS_SCRIPT),
                    "--before", str(roi_output),
                    "--after", str(output_video),
                    "--src_root_path", str(args.src_root_path),
                    "--output_json", str(boundary_metrics_path),
                ])
                _write_log(logs_dir, "local_edge_v1_boundary_metrics", boundary)
                roi_metrics_path = suite_dir / "local_edge_v1_roi_metrics.json"
                ...
                _write_log(logs_dir, "local_edge_v1_roi_metrics", roi)
                local_metrics_path = suite_dir / "local_edge_v1_local_metrics.json"
                local = _run([
                    PYTHON, str(LOCAL_EDGE_METRICS_SCRIPT),
                    "--before", str(roi_output),
                    "--after", str(output_video),
                    "--src_root_path", str(args.src_root_path),
                    "--debug_dir", str(roi_debug_dir),
                    "--output_json", str(local_metrics_path),
                ])
                _write_log(logs_dir, "local_edge_v1_local_metrics", local)
                row["boundary_metrics"] = _read_json(boundary_metrics_path)
                row["roi_metrics"] = _read_json(roi_metrics_path)
                row["local_edge_metrics"] = _read_json(local_metrics_path)
        rows.append(row)
