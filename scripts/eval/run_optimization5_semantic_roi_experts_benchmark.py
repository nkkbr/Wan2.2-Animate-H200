...
def _build_generate_command(args, run_dir: Path, mode: str):
    return [
        PYTHON,
        str(GENERATE_SCRIPT),
        "--task", "animate-14B",
        ...
        "--save_file", str(run_dir / "outputs" / f"{mode}.mkv"),
        "--output_format", "ffv1",
...
def main():
    parser = argparse.ArgumentParser(description="Run Optimization5 Step06 semantic ROI experts benchmark.")
    ...
    suite_name = args.suite_name or f"optimization5_step06_ab_{timestamp}"
    ...
    rows = []
    outputs = {}
    for mode in ("none", "semantic_experts_v1"):
        run_dir = suite_dir / mode
        generate = _run(_build_generate_command(args, run_dir, mode))
        _write_log(logs_dir, f"{mode}_generate", generate)
        row = {
            "variant": mode,
        ...
        if generate.returncode == 0 and output_video is not None:
            repl_metrics_path = suite_dir / f"{mode}_replacement_metrics.json"
            repl = _run([
                PYTHON,
            ...
            _write_log(logs_dir, f"{mode}_replacement_metrics", repl)
            ...
            outputs[mode] = output_video
        rows.append(row)

    semantic_comparisons = {}
    if "none" in outputs and "semantic_experts_v1" in outputs:
        metrics_path = suite_dir / "semantic_experts_v1_vs_none_boundary_metrics.json"
        semantic = _run([
            PYTHON,
            str(SEMANTIC_METRICS_SCRIPT),
            "--before", str(outputs["none"]),
            "--after", str(outputs["semantic_experts_v1"]),
            "--src_root_path", str(args.src_root_path),
            "--output_json", str(metrics_path),
        ])
        _write_log(logs_dir, "semantic_experts_v1_vs_none_boundary_metrics", semantic)
        semantic_comparisons["semantic_experts_v1_vs_none"] = _read_json(metrics_path)
