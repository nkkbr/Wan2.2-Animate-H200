...
def main():
    parser = argparse.ArgumentParser(description="Run Optimization4 Step03 legacy vs rich_v1 generate AB.")
    ...
    suite_name = args.suite_name or f"optimization4_step03_ab_{timestamp}"
    ...
    rows = []
    outputs = {}
    for conditioning_mode in ("legacy", "rich_v1"):
        stem = conditioning_mode
    ...
    if "legacy" in outputs and "rich_v1" in outputs:
        boundary_metrics_path = suite_dir / "rich_v1_vs_legacy_boundary_metrics.json"
        boundary = _run([
            PYTHON,
        ...
        _write_log(logs_dir, "rich_v1_vs_legacy_boundary_metrics", boundary)
