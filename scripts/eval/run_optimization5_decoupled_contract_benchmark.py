...
def main():
    parser = argparse.ArgumentParser(description="Run legacy vs decoupled_v1 generate AB for Optimization5 Step03.")
    ...
    parser.add_argument("--decoupled_boundary_strength", type=float, default=0.35)
    args = parser.parse_args()

    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    suite_name = args.suite_name or f"optimization5_step03_ab_{timestamp}"
    ...
    rows = []
    outputs = {}
    mask_path = Path(args.src_root_path) / "src_mask.mp4"
    for conditioning_mode in ("legacy", "decoupled_v1"):
        run_dir = suite_dir / conditioning_mode
        boundary_strength = args.decoupled_boundary_strength if conditioning_mode == "decoupled_v1" else 0.5
        result = _run(_build_generate_command(args, run_dir, conditioning_mode, boundary_strength))
        _write_log(logs_dir, f"{conditioning_mode}_generate", result)
        row = {
            "conditioning_mode": conditioning_mode,
            "run_dir": str(run_dir.resolve()),
            "generate_returncode": int(result.returncode),
            "replacement_boundary_strength": float(boundary_strength),
        }
    ...
    if "legacy" in outputs and "decoupled_v1" in outputs:
        boundary_metrics_path = suite_dir / "decoupled_v1_vs_legacy_boundary_metrics.json"
        boundary = _run([
            PYTHON,
            str(BOUNDARY_METRICS_SCRIPT),
            "--before", str(outputs["legacy"]),
            "--after", str(outputs["decoupled_v1"]),
            "--src_root_path", str(args.src_root_path),
            "--output_json", str(boundary_metrics_path),
        ])
        _write_log(logs_dir, "decoupled_v1_vs_legacy_boundary_metrics", boundary)
