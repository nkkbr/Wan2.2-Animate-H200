...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Optimization4 Step03 legacy vs rich_v1 AB.")
    ...
    seam_candidate = float(rich.get("seam_score_mean", 0.0) or 0.0)
    bg_baseline = float(legacy.get("background_fluctuation_mean", 0.0) or 0.0)
    bg_candidate = float(rich.get("background_fluctuation_mean", 0.0) or 0.0)
    ...
    gates = {
        "halo_reduction_ge_10pct": halo_reduction_pct >= 10.0,
        "gradient_gain_ge_6pct": gradient_gain_pct >= 6.0,
        "contrast_gain_ge_6pct": contrast_gain_pct >= 6.0,
