...
COMPUTE_RGBA_METRICS = REPO_ROOT / "scripts" / "eval" / "compute_rgba_foreground_metrics.py"
INFER_SCRIPT = REPO_ROOT / "scripts" / "eval" / "infer_rgba_foreground_proto.py"
EVAL_SCRIPT = REPO_ROOT / "scripts" / "eval" / "evaluate_optimization9_rgba_benchmark.py"
...
def _write_proxy_smoke(*, suite_dir: Path, name: str, src_root_path: Path, source_video_path: Path, logs_dir: Path) -> tuple[Path, Path]:
    ...
    if "rgba_composite_preview" in artifacts:
        proxy = load_rgb_artifact(src_root_path / artifacts["rgba_composite_preview"]["path"], artifacts["rgba_composite_preview"].get("format")).astype(np.uint8)[:frame_count]
        mode = "rgba_composite_preview"
    else:
        proxy = np.clip(
        ...
        mode = "paired_fg_alpha_composite"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Optimization9 Step04 RGBA foreground benchmark.")
    ...
    suite_name = args.suite_name or f"optimization9_step04_round{args.round}_{timestamp}"
    ...
    candidate_preprocess = suite_dir / "preprocess_rgba"
    infer = _run(
        [
            PYTHON,
            str(INFER_SCRIPT),
    ...
    _write_log(logs_dir, "infer_rgba_proto", infer)
    ...
    rgba_metrics = suite_dir / "rgba_vs_baseline_metrics.json"
    rgba = _run(
        [
            PYTHON,
            str(COMPUTE_RGBA_METRICS),
            "--before",
            str(baseline_video),
            "--after",
            str(candidate_video),
    ...
    _write_log(logs_dir, "rgba_metrics", rgba)
    ...
    summary = {
        ...
        "rgba_metrics": str(rgba_metrics.resolve()),
