...
def _run(cmd: list[str], *, logs_dir: Path, stem: str):
    logs_dir.mkdir(parents=True, exist_ok=True)
    stdout_path = logs_dir / f"{stem}.stdout.log"
    stderr_path = logs_dir / f"{stem}.stderr.log"
    with stdout_path.open("w", encoding="utf-8") as stdout_file, stderr_path.open("w", encoding="utf-8") as stderr_file:
        return subprocess.run(cmd, cwd=str(REPO_ROOT), text=True, stdout=stdout_file, stderr=stderr_file)
...
def _find_output_video(run_dir: Path):
    ...
    for suffix in ("*.mkv", "*.mp4"):
        matches = sorted(outputs.glob(suffix))
        if matches:
            return matches[0]
    return None


def _build_generate_command(args, run_dir: Path, conditioning_mode: str, boundary_strength: float):
    return [
        PYTHON,
        str(GENERATE_SCRIPT),
        "--task", "animate-14B",
        ...
        "--offload_model", "False",
        "--output_format", "ffv1",
        "--save_file", str(run_dir / "outputs" / f"{conditioning_mode}.mkv"),
        "--save_debug_dir", str(run_dir / "debug" / "generate"),
        "--replacement_conditioning_mode", conditioning_mode,
        "--boundary_refine_mode", "none",
        "--replacement_boundary_strength", str(boundary_strength),
        "--base_seed", str(args.base_seed),
    ]


def main():
    parser = argparse.ArgumentParser(description="Run Optimization6 Step03 decoupled contract/core-path benchmark.")
    ...
    parser.add_argument("--decoupled_v1_boundary_strength", type=float, default=0.35)
    parser.add_argument("--decoupled_v2_boundary_strength", type=float, default=0.50)
    args = parser.parse_args()

    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    suite_name = args.suite_name or f"optimization6_step03_ab_{timestamp}"
    ...
    boundary_strengths = {
        "legacy": 0.5,
        "decoupled_v1": float(args.decoupled_v1_boundary_strength),
        "decoupled_v2": float(args.decoupled_v2_boundary_strength),
    }
    for conditioning_mode in ("legacy", "decoupled_v1", "decoupled_v2"):
        run_dir = suite_dir / conditioning_mode
        generate = _run(
            _build_generate_command(args, run_dir, conditioning_mode, boundary_strengths[conditioning_mode]),
            logs_dir=logs_dir,
            stem=f"{conditioning_mode}_generate",
        )
        row = {
            "conditioning_mode": conditioning_mode,
            "run_dir": str(run_dir.resolve()),
            "generate_returncode": int(generate.returncode),
            "replacement_boundary_strength": boundary_strengths[conditioning_mode],
        }
        ...
        if generate.returncode == 0 and output_video is not None:
            ...
            ], logs_dir=logs_dir, stem=f"{conditioning_mode}_replacement_metrics")
    ...
    comparisons = {}
    for conditioning_mode in ("decoupled_v1", "decoupled_v2"):
        if "legacy" in outputs and conditioning_mode in outputs:
            ...
            ], logs_dir=logs_dir, stem=f"{conditioning_mode}_vs_legacy_boundary_metrics")
